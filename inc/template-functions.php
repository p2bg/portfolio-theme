 ...
 * @return array
 */
function p2bg_theme_body_classes( $classes ) {
...
add_filter( 'body_class', 'p2bg_theme_body_classes' );
 ...
 * @return array
 */
function p2bg_theme_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'p2bg_theme_post_classes', 10, 3 );
 ...
 * @return void
 */
function p2bg_theme_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'p2bg_theme_pingback_header' );
 ...
 * @return void
 */
function p2bg_theme_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'p2bg_theme_supports_js' );
 ...
 * @return array
 */
function p2bg_theme_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'p2bg_theme_comment_form_defaults' );
 ...
 * @return bool
 */
function p2bg_theme_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'p2bg_theme_can_show_post_thumbnail',
 ...
 * @return int
 */
function p2bg_theme_get_avatar_size() {
	return 60;
}
 ...
 * @since Twenty Twenty-One 1.0
 */
function p2bg_theme_continue_reading_text() {
 ...
 * @since Twenty Twenty-One 1.0
 */
function p2bg_theme_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . p2bg_theme_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'p2bg_theme_continue_reading_link_excerpt' );
 ...
 * @since Twenty Twenty-One 1.0
 */
function p2bg_theme_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . p2bg_theme_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'p2bg_theme_continue_reading_link' );

if ( ! function_exists( 'p2bg_theme_post_title' ) ) {
	/**
 ...
 * @return string
 */
function p2bg_theme_get_icon_svg( $group, $icon, $size = 24 ) {
	return p2bg_theme_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function p2bg_theme_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? p2bg_theme_get_icon_svg( 'ui', 'arrow_right' ) : p2bg_theme_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? p2bg_theme_get_icon_svg( 'ui', 'arrow_left' ) : p2bg_theme_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'p2bg_theme_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function p2bg_theme_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'p2bg_theme_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'p2bg_theme_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'p2bg_theme_generate_css' ) ) {
	...
	return p2bg_theme_generate_css( // @phpstan-ignore-line.
 ...
 */
function p2bg_theme_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function p2bg_theme_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'p2bg_theme_password_form', 10, 2 );
 ...
 */
function p2bg_theme_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'p2bg_theme_get_attachment_image_attributes', 10, 3 );
